...
public:
	...
	~Animation();
    bool DoUpdateStep(int motionType, double stepLengthRatio, double stepDurationRatio,int mode=1);
	//added by david
	void rotate_character(double angle);
	bool Update(int motionType, double stepLengthRatio, double stepDurationRatio, int mode=1);
	bool firstCycle;
    vector<VectorR3> targets_;

	void move_joint_by_delta(double dx, double dy, double dz, Joint_Name jn);
	void move_joint(Point3d pt, Joint_Name jn);
	//for two-link inverse kinematic
	Point3d compute_twoLinkIK(Point3d pt, Point3d p1, Point3d p2, Point3d p3,
...
private:
	...
	//Point3d lFootPosition, rFootPosition;
	double lx, lz, rx, rz, nlx, nlz, nrx, nrz;
	double nx, nz, rootx, rootz, nrootx, nrootz;

	void compute_joint_parent();
