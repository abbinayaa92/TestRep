...
void Animation::BuildSkeletonFromBVH(const char *fileName)
{
	...
	//node_.resize(id+1);
	//cout<<"id:"<<id<<"  size:"<<node_.size()<<endl;
    file.close();
}
...
bool Animation::Update(int motionType, double stepLengthRatio, double stepDurationRatio)
{
    ...
    tree_.Compute();
    double dLeft = node_[LEFT_TOE].GetGlobalPosition().y;
    double dRight = node_[RIGHT_TOE].GetGlobalPosition().y;
    if (dLeft < 0) {
        Quaternion2 rot;
    ...
    if (!firstCycle) {
        ...
        if (fabs(footPos.y) < 0.5 && isRootMovedLastFrame) { // only move the root if the foot is in contact with the ground fabs(footPos.y) < 0.5 && isRootMovedLastFrame
